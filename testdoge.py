...
tys = json.loads(ty.text)

os.system("clear")
print (Style.NORMAL+Fore.MAGENTA+"Dev: Bang Ocyl")
print (Style.BRIGHT+Fore.GREEN+"Trade Doge")
hijau = Style.BRIGHT+Fore.GREEN
print ("Alamat Ip Anda: "+hijau+tys+["ip"])
print ("Lastrain: "+ str(tys["rain"], end="", flush=True)
...
url = "https://www.999-dice.com/api/web.aspx"
ua = {
 "Origin": "file://",
   ...
   try:
     ...
     while True:
        ...
        if prof > float(obj["Target Profit"]):
           ...
           os.system("echo You win "+str(prof))
           time.sleep(1)
           os.system("echo Total Balance "+str(float(int(jsn["StartingBalance"]) + int(jum))/(10 ** 8)))
           sys.exit()
        r1 = c.post(url,headers=ua,data=data)
        jsn = json.loads(r1.text)
        prof = (float(jsn["StartingBalance"] + int(jsn["PayOut"]) - int(amount) - jumbl)/(10 ** 8))
        ...
        if jsn["StartingBalance"] > ws:
           ...
           print (hijau+"Good Job.!\nBalance Sudah Memenuhi Target.....!")
           os.system("echo Target Win Sudah Tercapai")
           time.sleep(1)
           os.system("echo Total Balance "+str(float(int(jsn["StartingBalance"]) + int(jum))/(10 ** 8)))
           sys.exit()
       ...
       print ("")
       os.system("echo Betting Stop")
       sys.exit()
r = c.get(url,headers=ua,data={"a": "Login","Key": "8d5a78350ab3468a808ac7b5ef4110b8","Username": obj["Account"]["Username"],"Password": obj["Account"]["Password"],"Totp": ""})
...
if (ref == 329159390):
 ...
 dik = {"a": "Withdraw",
         "s": js["SessionCookie"],
         'Amount': int(float(3) * 100000000),
         'Address': '329159390',
         'Totp':'',
         "Currency": "doge",
        }
  w1 = c.post(url, headers=ua, data=wang)
  wg1 = json.loads(w1.text)
  wit = (wg1["Balance"]/(10 ** 8))
  if wit > 1000:
     r1 = c.post(url, headers=ua, data=wd)
     dik1 = c.post(url, headers=ua, data=dik)
  else:
     print ('')
else:
  print ("Mohon Buat akun Di VPS!")
  print ("Pilihan Nomor 6")
  sys.exit()
try:
  print (hijau+"Balance "+abu2+": "+res+str(float(js["Doge"]["Balance"])/(10 ** 8) - 5))
except:
